...
//===----------------------------------------------------------------------===//
//
//  This is clang's ASTStructuralEquivalence.cpp, copied rather than
//  retold, so that a link compares two declarations the way a
//  compilation does and not the way someone remembered it doing.  The
//  class is renamed so that both can be linked into one program.
//
//  One thing here is a decision of ours, and it is marked where it is
//  made, in CheckKindSpecificEquivalence: a record and a C++ record are
//  compared as records rather than refused for being different kinds of
//  declaration.  That is the whole of the difference between comparing
//  two units of one language, which is what clang does, and comparing
//  two units of a program, which may be written in both.
//
//===----------------------------------------------------------------------===//
//
//  This file implement LinkEquivalenceContext class and helper functions
//  for layout matching.
//
...
// function:
// ```
// static bool IsStructurallyEquivalent(LinkEquivalenceContext &Context,
//                                      Decl *D1, Decl *D2);
// ```
...
// IsStructurallyEquivalent(LinkEquivalenceContext &Context, Decl *D1,
...
class StmtComparer {
  LinkEquivalenceContext &Context;
...
public:
  StmtComparer(LinkEquivalenceContext &C) : Context(C) {}
...
static bool
CheckStructurallyEquivalentAttributes(LinkEquivalenceContext &Context,
...
/// types.
static bool IsArrayStructurallyEquivalent(LinkEquivalenceContext &Context,
...
static bool IsEquivalentExceptionSpec(LinkEquivalenceContext &Context,
  ...
  // Between a unit read as C and a unit read as C++ the specification
  // does not say the two are different functions, because C has none to
  // say it with: the C++ declaration of a C function carries throw()
  // from the header it was read out of and the C declaration of the
  // same function does not.  What a linker joins them by -- the name --
  // does not mention it either.
  if (Context.FromCtx.getLangOpts().CPlusPlus !=
      Context.ToCtx.getLangOpts().CPlusPlus)
    return true;
...
static bool IsStructurallyEquivalent(LinkEquivalenceContext &Context,
  ...
  // And where one of the two is anonymous and the other is not, when what
  // each of them is, is a record with no name of its own.  Giving that
  // record a member to be reached through is a way of naming a place, and
  // a header that describes one struct to both languages names it in the
  // way each of them can: anonymous where C is reading and through a
  // member where C++ is.  The members are compared below either way, so
  // two records that really differ still come out different.
  if (Field1->isAnonymousStructOrUnion() != Field2->isAnonymousStructOrUnion()) {
    const auto *T1 = Field1->getType()->getAs<RecordType>();
    const auto *T2 = Field2->getType()->getAs<RecordType>();

    if (T1 && T2 && !T1->getDecl()->getIdentifier() &&
        !T2->getDecl()->getIdentifier())
      return IsStructurallyEquivalent(Context, T1->getDecl(), T2->getDecl());
  }
  ...
  if ((Field1->isBitField() || Field2->isBitField()) &&
    ...
    if (Diagnose && Context.Complain) {
      ...
      Context.Diag2(
          ...
          Context.getApplicableDiagnostic(diag::err_odr_tag_type_inconsistent))
          << Owner2Type << (&Context.FromCtx != &Context.ToCtx);
      DiagNote(Field2, &LinkEquivalenceContext::Diag2);
      DiagNote(Field1, &LinkEquivalenceContext::Diag1);
    }
    return false;
  }
...
static bool
IsStructurallyEquivalentLambdas(LinkEquivalenceContext &Context,
...
static bool
IsRecordContextStructurallyEquivalent(LinkEquivalenceContext &Context,
  ...
  while (true) {
    ...
    // The same allowance as in CheckKindSpecificEquivalence, and for
    // the same reason: a record read by C and the same record read by
    // C++ are different kinds of declaration and one context.  Without
    // it every anonymous member of a shared struct is refused, since
    // an anonymous one is looked for by where it sits and the place is
    // the enclosing record.
    if (DC1->getDeclKind() != DC2->getDeclKind() &&
        !(isa<RecordDecl>(DC1) && isa<RecordDecl>(DC2)))
      return false;
...
static bool IsStructurallyEquivalent(LinkEquivalenceContext &Context,
  ...
  if (!D1->getDeclName() && !D2->getDeclName()) {
    ...
    if (UnsignedOrNone Index1 =
            LinkEquivalenceContext::findUntaggedStructOrUnionIndex(D1)) {
      if (UnsignedOrNone Index2 =
              LinkEquivalenceContext::findUntaggedStructOrUnionIndex(
                  D2)) {
        if (*Index1 != *Index2)
          return false;
      }
  ...
  if (Spec1 && Spec2) {
    ...
    // And that is the whole of what a specialisation is.  A template is
    // made into something once per thing it is made of, and which of
    // its members have been made along with it is not part of what it
    // is: a unit that called begin() and a unit that called size() hold
    // std::vector<int> with different members made, and they hold the
    // same type.  Comparing the members would say otherwise, and would
    // say it of every specialisation two units share.
    return true;
  }
  ...
  if (Context.EqKind == LinkEquivalenceKind::Minimal)
...
static bool IsTemplateDeclCommonStructurallyEquivalent(
    LinkEquivalenceContext &Ctx, TemplateDecl *D1, TemplateDecl *D2) {
  if (!IsStructurallyEquivalent(D1->getIdentifier(), D2->getIdentifier()))
    return false;
...
DiagnosticBuilder LinkEquivalenceContext::Diag1(SourceLocation Loc,
                                                      unsigned DiagID) {
...
DiagnosticBuilder LinkEquivalenceContext::Diag2(SourceLocation Loc,
                                                      unsigned DiagID) {
  assert(Complain && "Not allowed to complain");
  if (!LastDiagFromC2)
...
UnsignedOrNone
LinkEquivalenceContext::findUntaggedStructOrUnionIndex(RecordDecl *Anon) {
...
unsigned LinkEquivalenceContext::getApplicableDiagnostic(
...
bool LinkEquivalenceContext::IsEquivalent(Decl *D1, Decl *D2) {
...
bool LinkEquivalenceContext::IsEquivalent(QualType T1, QualType T2) {
  assert(DeclsToCheck.empty());
  assert(VisitedDecls.empty());
...
bool LinkEquivalenceContext::IsEquivalent(Stmt *S1, Stmt *S2) {
  assert(DeclsToCheck.empty());
  assert(VisitedDecls.empty());
...
bool LinkEquivalenceContext::CheckCommonEquivalence(Decl *D1, Decl *D2) {
...
bool LinkEquivalenceContext::CheckKindSpecificEquivalence(
    Decl *D1, Decl *D2) {
...
bool LinkEquivalenceContext::Finish() {
